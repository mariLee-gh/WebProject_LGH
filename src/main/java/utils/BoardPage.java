...
public class BoardPage {
	 ...
	 * blockPage, : 위와동일. 한 블럭당 출력할 페이지 번호수
	 * pageNum, : 현재 진입해있는 페이지 번호
	 * reqUrl : 게시판 목록의 요청명 혹은 현제페이지 URL
	 * */
	...
	public static String pagingStr(int totalCount, int pageSize, int blockPage, int pageNum, String reqUrl) {
		...
		/*
		 * 전체페이지 수 계산
		 * : ceil(전체게시물 수/ 한페이지당 게시물 수) => 결과를 무조건 올림처리하려 계산한다.
		 * */
		int totalPages = (int) (Math.ceil(((double) totalCount/ pageSize)));
		//'이전 페이지 블록 바로가' 출력
		
		/*
		 * 페이지 블럭에서 첫번째 페이지 번호를 구하기 위한 계산식.
		 * 우리는 blockPage를 5로 설정했으므로 아래 계산식에 pageNum=1~5를 대입해보면 1의 결과가 나온다.
		 * 즉, 1~5페이지 일때는 pageTemp가 1이되어 반복하면서 출력할 수 있다.
		 * */
		int pageTemp = (((pageNum -1) / blockPage) * blockPage) +1;
		//첫번재 블럭이 아닌 경우에만 이전페이지로 바로가기 링크를 출력한다.
		if(pageTemp != 1) {

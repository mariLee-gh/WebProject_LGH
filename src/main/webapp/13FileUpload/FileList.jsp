...
<body>
	...
	<%
	//업로드 디렉토리 물리적(혹은 절대 )경로 얻어오기
	String saveDirectory = application.getRealPath("/Uploads");
	//해당경로를 통해 File인스턴스 생성
	File file = new File(saveDirectory);
	//디텍토리에 저장된 파일의 목록을 배열로 얻어온다.
 ...
 <table border="1">
   ...
   <%
   int fileCnt = 1;
   //얻어온 파일 목록의 갯수만큼 확장 for문을 통해 반복
   for (File f : fileList){
   %>
      <tr>
         <td><%=fileCnt %></td>
         ...
         <!-- 파일 다운로드 링크 -->
         <td><a href="Download.jsp?oName=myImage.jpg&sName=<%=f.getName()%>">[다운로드]</td>
      </tr>
   <%
      fileCnt++;
   }
